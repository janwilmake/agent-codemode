...
Examples:
  ...
  agent-codemode call linear list_issues --json '{"team":"Hyre Ops","limit":5}'
...
async function main(argv: string[]): Promise<number> {
  ...
  if (command === "tools") {
    const server = rest[0];
    if (!server) throw new Error("usage: agent-codemode tools <server>");
  ...
  if (command === "types") {
    ...
    if (rest.includes("--all")) {
      ...
      for (const s of servers) {
        const session = await openSession(s.name);
        try {
          const tools = await session.listTools();
    ...
    if (urlArg) {
      ...
      throw new Error("usage: agent-codemode types <server> [--out file] | --all | --url <url> [--name n]");
    }
  ...
  if (command === "call") {
    ...
    if (!server || !tool) throw new Error("usage: agent-codemode call <server> <tool> [args]");
