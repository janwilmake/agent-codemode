...
import { shortName } from "./resolver.js";

/** The published name — generated modules import types from it and augment it. */
const PACKAGE = "agent-codemode";
...
export function generateModule(serverName: string, tools: ToolDefinition[]): GeneratedModule {
  ...
  parts.push(`// Generated by ${PACKAGE} from ${serverName}'s tools/list. Do not edit by hand.`);
  parts.push(`import type { ToolResult } from "${PACKAGE}";`);
  parts.push("");
  ...
  // Teach `mcp` about this server, so callers get types with no cast at all:
  // just import this module for its side effect. Both the wire name and the
  // short alias are registered, because the runtime resolves both.
  const keys = [serverName, shortName(serverName)].filter((k, i, a) => a.indexOf(k) === i);
  parts.push(`declare module "${PACKAGE}" {`);
  parts.push(`  interface McpServers {`);
  for (const key of keys) parts.push(`    ${quoteKey(key)}: ${iface}Client;`);
  parts.push(`  }`);
  parts.push(`}`);
  parts.push("");
